...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    this.scale = (this.screen?.height || 600) / 1500;
    this.plane.scale.y = (this.viewport.height * (900 * this.scale)) / (this.screen?.height || 600);
    this.plane.scale.x = (this.viewport.width * (700 * this.scale)) / (this.screen?.width || 800);
...
class App {
  ...
  createRenderer() {
    try {
      ...
      console.warn('CircularGallery: WebGL renderer failed to initialize', err);
      this.gl = null;
    }
  }
  createCamera() {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    ...
    this.rawGalleryItems = galleryItems;
    ...
    this.medias = this.mediasImages.map((data, index) => {
      ...
      return new Media({
        ...
        borderRadius,
        font,
        rawItem: galleryItems[itemIndex],
        itemIndex
      });
    });
  }
  ...
  onTouchMove(e) {
    ...
    const clientX = e.touches ? e.touches[0].clientX : e.clientX;
    const clientY = e.touches ? e.touches[0].clientY : e.clientY;
    const dist = Math.hypot(clientX - this.downPos.x, clientY - this.downPos.y);
    if (dist > 8) {
  ...
  onTouchUp(e) {
    ...
    const dist = this.downPos ? Math.hypot(clientX - this.downPos.x, clientY - this.downPos.y) : 999;
    const timeElapsed = Date.now() - (this.downTime || 0);

    // If it was a clean click/tap without dragging, trigger card click
    if (this.isDown && !this.isDragging && dist < 12 && timeElapsed < 600) {
  ...
  handleClick(clientX, clientY) {
    if (!this.container || !this.medias || !this.medias.length || !this.viewport) return;
    ...
    // World Coordinates at camera depth z=0
    ...
    for (const media of this.medias) {
      if (!media.plane || !media.plane.position) continue;

      // Transform world (worldX, worldY) to media plane local space
      ...
      const halfW = (media.plane.scale.x / 2) * 1.1;
      const topY = (media.plane.scale.y / 2) * 1.05;
      const bottomY = -media.plane.scale.y / 2 - media.plane.scale.y * 0.4; // card body + title text mesh

      // Check if click point is inside the card's bounding rectangle
      if (localX >= -halfW && localX <= halfW && localY >= bottomY && localY <= topY) {
        const dist = Math.hypot(localX, localY);
        const frontBias = Math.abs(media.plane.position.x) * 0.4;
        const totalScore = dist + frontBias;
    ...
    if (closestMedia && closestMedia.rawItem) {
      if (typeof this.onItemClick === 'function') {
        this.onItemClick(closestMedia.rawItem, closestMedia.itemIndex);
      }
    }
  }
  onWheel(e) {
  ...
  update() {
    if (!this.gl || !this.renderer || !this.scene || !this.camera) return;
...
export default function CircularGallery({
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="region"
      aria-label="Circular image gallery. Click any card to navigate or use left and right arrow keys to navigate."
    />
  );
}
