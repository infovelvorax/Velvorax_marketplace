...
const Loader = ({ placeholderSrc }) => {
  ...
  return (
    <Html center>
      {placeholderSrc ? (
        <img src={placeholderSrc} width={128} height={128} style={{ filter: 'blur(8px)', borderRadius: 8 }} alt="Loading 3D preview" />
      ) : (
        <div className="model-viewer-loader-badge">
          <span className="model-viewer-spinner" />
...
const createShadowTexture = () => {
  ...
  // Radial gradient: rich dark center smoothly diffusing out to transparent
  const gradient = ctx.createRadialGradient(256, 256, 0, 256, 256, 256);
  gradient.addColorStop(0, 'rgba(0, 0, 0, 0.85)');
  gradient.addColorStop(0.2, 'rgba(0, 0, 0, 0.60)');
...
/**
 * Pure Transparent 3D Spinning PNG Logo Component
 * Renders large transparent PNG logo rotating smoothly in 3D space with a realistic ground shadow below it.
 */
const Logo3DInner = ({
  logoSrc = '/velvorax-logo.png',
  xOff = 0,
  yOff = 0,
  pivot,
  initYaw = 0,
  ...
  const shadowWideRef = useRef(null);
  const shadowCoreRef = useRef(null);
  ...
  // Generate smooth high-resolution blurred shadow texture
  ...
  useLayoutEffect(() => {
    if (outer.current) {
      ...
      outer.current.getWorldPosition(pivotW.current);
      pivot.copy(pivotW.current);
    }
    onLoaded?.();
  }, [initPitch, initYaw, pivot, onLoaded]);
  ...
  useFrame((state, dt) => {
    ...
    // Dynamic floating bob and responsive floor shadow
    const bob = Math.sin(state.clock.elapsedTime * 1.6) * 0.12;
    if (logoGroup.current) {
      logoGroup.current.position.y = 0.35 + bob;
      need = true;
    }

    // Ground shadow below reacts dynamically to logo floating altitude
    if (shadowWideRef.current) {
      const sFactor = 1 + bob * 0.35;
      shadowWideRef.current.scale.set(sFactor, sFactor, 1);
      if (shadowWideRef.current.material) {
        shadowWideRef.current.material.opacity = Math.max(0.2, 0.70 - bob * 1.0);
      }
      need = true;
    }
    if (shadowCoreRef.current) {
      const sFactor = 1 + bob * 0.25;
      shadowCoreRef.current.scale.set(sFactor, sFactor, 1);
      if (shadowCoreRef.current.material) {
        shadowCoreRef.current.material.opacity = Math.max(0.2, 0.55 - bob * 1.2);
      }
      need = true;
    }
  ...
  return (
    <group ref={outer}>
      {/* Big Size Pure Transparent PNG Logo Group */}
      <group ref={logoGroup} position={[0, 0.35, 0]}>
        
        {/* Front Face of Pure Transparent PNG Logo */}
        ...
        {/* Back Face of Pure Transparent PNG Logo */}
      ...
      {/* Realistic Soft Ambient Floor Shadow Beneath Floating Logo */}
      {/* 1. Diffuse Wide Outer Shadow */}
      <mesh
        ref={shadowWideRef}
        position={[0, -1.95, 0]}
      ...
      {/* 2. Dense Core Center Contact Shadow */}
      <mesh
        ref={shadowCoreRef}
        position={[0, -1.94, 0]}
        rotation={[-Math.PI / 2, 0, 0]}
...
export const ModelViewer = ({
  ...
  const is3DLogo = isLogo || !url || url.endsWith('.png') || url.endsWith('.jpg') || url.endsWith('.webp');
  ...
  const capture = () => {
    ...
    a.download = 'velvorax-3d-logo.png';
    a.href = urlPNG;
    a.click();
    ...
    >
      {showScreenshotButton && (
        <button
          ...
          aria-label="Take 3D Screenshot"
        >
          📷 Snapshot
        </button>
      )}

      {/* Optional Interactive Helper Overlay Badge */}
      {showInteractiveBadge && (
        <div className="model-viewer-interactive-badge">
          <span className="model-viewer-3d-dot" />
      ...
      >
        ...
        <directionalLight position={[5, 6, 5]} intensity={keyLightIntensity} color="#FFFBEB" />
        <directionalLight position={[-5, 3, 4]} intensity={fillLightIntensity} color="#E0F2FE" />

        {!is3DLogo && (
          <ContactShadows ref={contactRef} position={[0, -2.25, 0]} opacity={0.55} scale={7} blur={2.6} far={4.5} color="#000000" />
        )}
