...
export const Services = () => {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      ...
      try {
        ...
        setError('Unable to load professional services at this time.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const serviceSubcategories = [
    ...
    'Packers & Movers',
    'Painters & Home Renovation',
    'Salon & Makeup at Home',
    'Carpentry & Furniture Repair',
    'Car & Bike Servicing',
    'Legal, CA & Business Filing',
    'Web Development & Graphic Design',
    'Event Planning & Photography'
  ];

  const serviceDeliveryTypes = [
    { label: '🏠 At Doorstep / Home', value: 'doorstep' },
    { label: '🏪 Workshop / Studio', value: 'workshop' },
    { label: '🌐 Online / Remote', value: 'remote' }
  ];

  const availabilityOptions = [
    { label: '⚡ Instant / Same Day', value: 'instant' },
    { label: '📅 Flexible Booking', value: 'flexible' },
    ...
    <div className="space-y-6">
      ...
      <div className="pt-4 border-t border-[var(--border-subtle)]">
        ...
        <div className="space-y-1.5">
          {serviceDeliveryTypes.map((sd) => (
            <button
              key={sd.value}
              type="button"
              onClick={() => setFilter('serviceType', serviceType === sd.value ? '' : sd.value)}
              className={cn(
                "w-full text-left px-3 py-1.5 rounded-lg text-xs font-bold transition-colors cursor-pointer",
                serviceType === sd.value
        ...
        <div className="space-y-1.5">
          {availabilityOptions.map((av) => (
            <button
              key={av.value}
              type="button"
              onClick={() => setFilter('availability', availability === av.value ? '' : av.value)}
              className={cn(
                "w-full text-left px-3 py-1.5 rounded-lg text-xs font-bold transition-colors cursor-pointer",
  ...
  return (
    <div className="bg-[var(--bg-primary)] text-[var(--text-primary)] min-h-screen py-8 sm:py-10 transition-colors duration-200">
      <Container size="7xl">
        ...
        {/* ============================================================ */}
        {/* TOP HERO BANNER & SERVICES PORTAL                            */}
        {/* ============================================================ */}
        <div className="relative mb-8 p-6 sm:p-10 rounded-3xl bg-gradient-to-br from-[var(--bg-surface)] via-[var(--bg-surface)] to-[var(--bg-secondary)] border border-[var(--border-primary)] shadow-sm overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-2">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-teal-500/10 border border-teal-500/20 text-teal-600 dark:text-teal-400 text-xs font-black uppercase tracking-wider">
                <span>🛠️</span>
                <span>Verified Local Experts & Pros Directory</span>
              </div>
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-[var(--text-primary)] tracking-tight">
                Hire Verified Local Professionals
              </h1>
              <p className="text-sm sm:text-base text-[var(--text-secondary)] max-w-2xl leading-relaxed">
                Repairs, deep cleaning, electricians, carpenters, AC technicians, packers & movers, salon experts, and web professionals with direct contact numbers.
              </p>
            </div>
          ...
          <div className="relative z-10 mt-8 pt-6 border-t border-[var(--border-subtle)] flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap gap-2">
              {[
                { label: '🛠️ All Services', value: '' },
                { label: '🧹 Home Cleaning', value: 'Home Cleaning & Pest Control' },
                { label: '❄️ AC & Appliance', value: 'AC & Appliance Repair' },
                { label: '📦 Packers & Movers', value: 'Packers & Movers' },
                { label: '💆 Salon at Home', value: 'Salon & Makeup at Home' }
              ].map((tab) => (
                <button
                  key={tab.label}
                  type="button"
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 bg-[var(--bg-surface)] p-4 rounded-2xl border border-[var(--border-primary)] shadow-2xs">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="text-xs sm:text-sm font-bold text-[var(--text-secondary)]">
              Showing <span className="text-[var(--text-primary)] font-black">{totalCount}</span> verified service pros
            </span>
            ...
            {activeFiltersCount > 0 && (
              <div className="hidden md:flex items-center gap-1.5 flex-wrap">
                {subcategory && (
                  <span className="px-2.5 py-0.5 rounded-md bg-teal-500/10 text-teal-600 dark:text-teal-400 border border-teal-500/20 text-[11px] font-bold">
                    {subcategory}
                  </span>
                )}
                {serviceType && (
                  <span className="px-2.5 py-0.5 rounded-md bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20 text-[11px] font-bold capitalize">
                    {serviceType}
                  </span>
                )}
                {city && (
        ...
        {viewMode === 'map' && (
          <div className="mb-8 rounded-3xl overflow-hidden border border-[var(--border-primary)] shadow-md bg-[var(--bg-surface)] p-2">
            <RealMapLocation
              city={city || 'Bengaluru'}
              address={`${city || 'All Cities'}, Local Service Network`}
              title="Explore Local Service Experts on Map"
              interactive={true}
              height="380px"
            />
          </div>
          ...
          <main className="flex-1 w-full min-w-0">
            {loading ? (
              <div className="py-28 flex flex-col items-center justify-center gap-3">
                <Loader size="lg" />
                <span className="text-xs font-bold text-[var(--text-muted)] animate-pulse">Loading verified service pros...</span>
              </div>
            ) : error ? (
              ...
              <EmptyState
                title="No service professionals found matching criteria"
                description="Try clearing your budget or subcategory filters to see all available service providers."
                action={
                  <button
                    type="button"
                    onClick={handleClearFilters}
        ...
        {mobileFiltersOpen && (
          <div className="fixed inset-0 z-50 lg:hidden flex justify-end" role="dialog" aria-modal="true">
            ...
            <div className="relative w-full max-w-sm bg-[var(--bg-surface)] h-full overflow-y-auto z-10 p-6 shadow-2xl flex flex-col justify-between animate-slide-left">
              <div>
                <div className="flex items-center justify-between pb-4 mb-4 border-b border-[var(--border-subtle)]">
                  <span className="text-base font-black text-[var(--text-primary)]">Filter Services</span>
                  <button
                    type="button"
                    onClick={() => setMobileFiltersOpen(false)}
