...
export const Vehicles = () => {
  ...
  useEffect(() => {
    const fetchVehicles = async () => {
      ...
      try {
        ...
        setError('Unable to load vehicle listings at this time.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const fuelTypes = [
    ...
    { label: 'CNG / LPG', value: 'cng' }
  ];

  const budgetPresets = [
    { label: 'Under ₹1 Lakh', min: '', max: '100000' },
    { label: '₹1L - ₹5 Lakhs', min: '100000', max: '500000' },
    { label: '₹5L - ₹15 Lakhs', min: '500000', max: '1500000' },
    { label: '₹15L - ₹30 Lakhs', min: '1500000', max: '3000000' },
    { label: '₹30 Lakhs+', min: '3000000', max: '' }
  ];

  const yearPresets = ['2024', '2023', '2022', '2020', '2018', '2015'];
        ...
        <div className="grid grid-cols-2 gap-2">
          {fuelTypes.map((f) => (
            <button
              key={f.value}
              type="button"
              onClick={() => setFilter('fuel', fuel === f.value ? '' : f.value)}
              className={cn(
                "px-2.5 py-2 rounded-xl text-xs font-bold transition-all text-center border cursor-pointer",
                fuel === f.value
        ...
        <div className="grid grid-cols-2 gap-2">
          {['Automatic', 'Manual'].map((t) => (
            <button
              key={t}
              type="button"
              onClick={() => setFilter('transmission', transmission === t ? '' : t)}
              className={cn(
                "px-2.5 py-2 rounded-xl text-xs font-bold transition-all text-center border cursor-pointer",
                transmission === t
                  ? "bg-[var(--button-primary)] text-[var(--button-primary-text)] border-[var(--button-primary)] shadow-xs"
        ...
        <div className="flex flex-wrap gap-1.5">
          {yearPresets.map((y) => (
            <button
              key={y}
              type="button"
              onClick={() => setFilter('year', year === y ? '' : y)}
              className={cn(
                "px-2.5 py-1.5 rounded-lg text-xs font-bold border transition-all cursor-pointer",
                year === y
        ...
        <div className="space-y-1.5">
          {[
            ...
            <button
              key={k.value}
              type="button"
              onClick={() => setFilter('maxKm', maxKm === k.value ? '' : k.value)}
              className={cn(
                "w-full text-left px-3 py-1.5 rounded-lg text-xs font-bold transition-colors cursor-pointer",
                maxKm === k.value
                  ? "bg-[var(--button-primary)] text-[var(--button-primary-text)]"
  ...
  return (
    <div className="bg-[var(--bg-primary)] text-[var(--text-primary)] min-h-screen py-8 sm:py-10 transition-colors duration-200">
      <Container size="7xl">
        ...
        {/* ============================================================ */}
        {/* TOP HERO BANNER & VEHICLES PORTAL                            */}
        {/* ============================================================ */}
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-2">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-600 dark:text-blue-400 text-xs font-black uppercase tracking-wider">
                <span>🚗</span>
                <span>Verified Auto & Motors Marketplace</span>
              </div>
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-[var(--text-primary)] tracking-tight">
                Buy & Sell Cars, Bikes & Vehicles
              </h1>
              <p className="text-sm sm:text-base text-[var(--text-secondary)] max-w-2xl leading-relaxed">
                Browse inspected pre-owned cars, electric scooters, commercial trucks, motorcycles, and spare parts with direct seller contact and real locations.
              </p>
            </div>
          ...
          <div className="relative z-10 mt-8 pt-6 border-t border-[var(--border-subtle)] flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap gap-2">
              {[
                ...
                { label: '🔑 Buy Vehicles', value: 'sell' },
                { label: '🏷️ For Lease / Rent', value: 'rent' }
              ].map((tab) => (
                <button
                  key={tab.value}
                  type="button"
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 bg-[var(--bg-surface)] p-4 rounded-2xl border border-[var(--border-primary)] shadow-2xs">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="text-xs sm:text-sm font-bold text-[var(--text-secondary)]">
              Showing <span className="text-[var(--text-primary)] font-black">{totalCount}</span> verified vehicles
            </span>
            ...
            {activeFiltersCount > 0 && (
              <div className="hidden md:flex items-center gap-1.5 flex-wrap">
                ...
                {fuel && (
                  <span className="px-2.5 py-0.5 rounded-md bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 text-[11px] font-bold capitalize">
                    {fuel}
                  </span>
                )}
                {city && (
        ...
        {viewMode === 'map' && (
          <div className="mb-8 rounded-3xl overflow-hidden border border-[var(--border-primary)] shadow-md bg-[var(--bg-surface)] p-2">
            <RealMapLocation
              city={city || 'Bengaluru'}
              address={`${city || 'All Cities'}, Auto & Vehicle Hub`}
              title="Explore Vehicles on Interactive Map"
              interactive={true}
              height="380px"
            />
          </div>
          ...
          <main className="flex-1 w-full min-w-0">
            {loading ? (
              <div className="py-28 flex flex-col items-center justify-center gap-3">
                <Loader size="lg" />
                <span className="text-xs font-bold text-[var(--text-muted)] animate-pulse">Loading verified vehicles...</span>
              </div>
            ) : error ? (
              ...
              <EmptyState
                title="No vehicles found matching criteria"
                description="Try clearing your budget, fuel or subcategory filters to see all available vehicles."
                action={
                  <button
                    type="button"
                    onClick={handleClearFilters}
        ...
        {mobileFiltersOpen && (
          <div className="fixed inset-0 z-50 lg:hidden flex justify-end" role="dialog" aria-modal="true">
            ...
            <div className="relative w-full max-w-sm bg-[var(--bg-surface)] h-full overflow-y-auto z-10 p-6 shadow-2xl flex flex-col justify-between animate-slide-left">
              <div>
                <div className="flex items-center justify-between pb-4 mb-4 border-b border-[var(--border-subtle)]">
                  <span className="text-base font-black text-[var(--text-primary)]">Filter Vehicles</span>
                  <button
                    type="button"
                    onClick={() => setMobileFiltersOpen(false)}
