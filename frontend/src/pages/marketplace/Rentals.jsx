...
export const Rentals = () => {
  ...
  useEffect(() => {
    const fetchRentals = async () => {
      ...
      try {
        ...
        setError('Unable to load rental listings at this time.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const rentalCategories = [
    { label: '🏢 House, Flat & Office Leases', value: 'properties' },
    { label: '🚗 Cars, Bikes & Commercial Rentals', value: 'vehicles' },
    { label: '🚜 Farm Machinery & Tractor Lease', value: 'farm' },
    { label: '📱 Laptops, Cameras & Tech Rentals', value: 'products' },
    { label: '🛋️ Furniture & Appliance Rentals', value: 'furniture' }
  ];

  const rentalFrequencies = [
    { label: '🗓️ Monthly Rent', value: 'monthly' },
    { label: '⏳ Daily / Weekly', value: 'daily' },
    { label: '📜 Long-Term Lease (1-3 yrs)', value: 'yearly' }
  ];

  const rentPresets = [
    { label: 'Under ₹5,000 / mo', min: '', max: '5000' },
    { label: '₹5k - ₹15,000 / mo', min: '5000', max: '15000' },
    { label: '₹15k - ₹35,000 / mo', min: '15000', max: '35000' },
    { label: '₹35k - ₹75,000 / mo', min: '35000', max: '75000' },
    { label: '₹75,000+ / mo', min: '75000', max: '' }
  ];
  ...
  const renderFilterContent = () => (
    <div className="space-y-6">
      ...
      <div className="pt-4 border-t border-[var(--border-subtle)]">
        ...
        <div className="space-y-1.5">
          {rentalFrequencies.map((rf) => (
            <button
              key={rf.value}
              type="button"
              onClick={() => setFilter('duration', duration === rf.value ? '' : rf.value)}
              className={cn(
  ...
  return (
    <div className="bg-[var(--bg-primary)] text-[var(--text-primary)] min-h-screen py-8 sm:py-10 transition-colors duration-200">
      <Container size="7xl">
        ...
        {/* ============================================================ */}
        {/* TOP HERO BANNER & RENTALS PORTAL                             */}
        {/* ============================================================ */}
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-2">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-600 dark:text-blue-400 text-xs font-black uppercase tracking-wider">
                <span>🔑</span>
                <span>Verified Rentals & Leasing Marketplace</span>
              </div>
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-[var(--text-primary)] tracking-tight">
                Rent Houses, Cars, Machinery & Gear
              </h1>
              <p className="text-sm sm:text-base text-[var(--text-secondary)] max-w-2xl leading-relaxed">
                Explore verified rental flats, independent house leases, commercial vehicles for hire, tractor leases, event sound equipment, and laptop rentals with direct contact numbers.
              </p>
            </div>
          ...
          <div className="relative z-10 mt-8 pt-6 border-t border-[var(--border-subtle)] flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap gap-2">
              {[
                ...
                { label: '🏢 Home & Property', value: 'properties' },
                { label: '🚗 Cars & Vehicles', value: 'vehicles' },
                { label: '🚜 Farm Machinery', value: 'farm' },
                { label: '📱 Laptops & Tech', value: 'products' }
              ].map((tab) => (
                <button
                  key={tab.label}
                  type="button"
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 bg-[var(--bg-surface)] p-4 rounded-2xl border border-[var(--border-primary)] shadow-2xs">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="text-xs sm:text-sm font-bold text-[var(--text-secondary)]">
              Showing <span className="text-[var(--text-primary)] font-black">{totalCount}</span> verified rental listings
            </span>
        ...
        {viewMode === 'map' && (
          <div className="mb-8 rounded-3xl overflow-hidden border border-[var(--border-primary)] shadow-md bg-[var(--bg-surface)] p-2">
            <RealMapLocation
              city={city || 'Bengaluru'}
              address={`${city || 'All Cities'}, Rental & Leasing Properties`}
              title="Explore Rental Items & Properties on Map"
              interactive={true}
              height="380px"
            />
          </div>
          ...
          <main className="flex-1 w-full min-w-0">
            {loading ? (
              <div className="py-28 flex flex-col items-center justify-center gap-3">
                <Loader size="lg" />
                <span className="text-xs font-bold text-[var(--text-muted)] animate-pulse">Loading verified rentals...</span>
              </div>
            ) : error ? (
              ...
              <EmptyState
                title="No rental listings found matching criteria"
                description="Try clearing your budget or category filters to see all available rental items."
                action={
                  <button
                    type="button"
                    onClick={handleClearFilters}
              ...
              <div className={cn(
                ...
                {listings.map((item) => (
                  <ListingCard
                    key={item._id}
                    {...item}
                    categorySlug={item.category?.slug || item.categorySlug || 'properties'}
                    viewMode={viewMode}
                  />
                ))}
              </div>
        ...
        {mobileFiltersOpen && (
          <div className="fixed inset-0 z-50 lg:hidden flex justify-end" role="dialog" aria-modal="true">
            ...
            <div className="relative w-full max-w-sm bg-[var(--bg-surface)] h-full overflow-y-auto z-10 p-6 shadow-2xl flex flex-col justify-between animate-slide-left">
              <div>
                <div className="flex items-center justify-between pb-4 mb-4 border-b border-[var(--border-subtle)]">
                  <span className="text-base font-black text-[var(--text-primary)]">Filter Rentals</span>
                  <button
                    type="button"
                    onClick={() => setMobileFiltersOpen(false)}
