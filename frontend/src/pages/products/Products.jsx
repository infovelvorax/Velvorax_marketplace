  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        setError('Unable to load electronics listings at this time.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const renderFilterContent = () => (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-1.5 max-h-36 overflow-y-auto pr-1">
          {popularBrands.map((b) => (
            <button
              key={b}
              type="button"
              onClick={() => setFilter('brand', brand === b ? '' : b)}
              className={cn(
      ...
      <div className="pt-4 border-t border-[var(--border-subtle)]">
        ...
        <div className="space-y-1.5">
          {['Brand Warranty', 'Seller Warranty', 'Any Warranty'].map((w) => (
            <button
              key={w}
              type="button"
              onClick={() => setFilter('warranty', warranty === w ? '' : w)}
              className={cn(
                "w-full text-left px-3 py-1.5 rounded-lg text-xs font-bold transition-colors cursor-pointer",
                warranty === w
                  ? "bg-[var(--button-primary)] text-[var(--button-primary-text)]"
  ...
  return (
    <div className="bg-[var(--bg-primary)] text-[var(--text-primary)] min-h-screen py-8 sm:py-10 transition-colors duration-200">
      <Container size="7xl">
        ...
        {/* ============================================================ */}
        {/* TOP HERO BANNER & PRODUCTS PORTAL                            */}
        {/* ============================================================ */}
        <div className="relative mb-8 p-6 sm:p-10 rounded-3xl bg-gradient-to-br from-[var(--bg-surface)] via-[var(--bg-surface)] to-[var(--bg-secondary)] border border-[var(--border-primary)] shadow-sm overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-2">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-600 dark:text-purple-400 text-xs font-black uppercase tracking-wider">
                <span>📱</span>
                <span>Verified Tech & Electronics Marketplace</span>
              </div>
              ...
              <p className="text-sm sm:text-base text-[var(--text-secondary)] max-w-2xl leading-relaxed">
                Explore genuine smartphones, Apple iPhones, MacBooks, gaming setups, smart TVs, sound systems, and home gadgets from verified sellers.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              >
                <span>➕</span>
                <span>Sell Electronics Free</span>
              </Link>
            </div>
          </div>
          ...
          <div className="relative z-10 mt-8 pt-6 border-t border-[var(--border-subtle)] flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap gap-2">
              {[
                ...
                { label: '⚡ Buy Gadgets', value: 'sell' },
                { label: '🏷️ Electronics Rental', value: 'rent' }
              ].map((tab) => (
                <button
                  key={tab.value}
                  type="button"
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 bg-[var(--bg-surface)] p-4 rounded-2xl border border-[var(--border-primary)] shadow-2xs">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="text-xs sm:text-sm font-bold text-[var(--text-secondary)]">
              Showing <span className="text-[var(--text-primary)] font-black">{totalCount}</span> verified tech items
            </span>
        ...
        {viewMode === 'map' && (
          <div className="mb-8 rounded-3xl overflow-hidden border border-[var(--border-primary)] shadow-md bg-[var(--bg-surface)] p-2">
            <RealMapLocation
              city={city || 'Bengaluru'}
              address={`${city || 'All Cities'}, Tech & Electronics Hub`}
              title="Explore Electronics Sellers on Interactive Map"
              interactive={true}
              height="380px"
            />
          </div>
          ...
          <main className="flex-1 w-full min-w-0">
            {loading ? (
              <div className="py-28 flex flex-col items-center justify-center gap-3">
                <Loader size="lg" />
                <span className="text-xs font-bold text-[var(--text-muted)] animate-pulse">Loading verified electronics...</span>
              </div>
            ) : error ? (
              ...
              <EmptyState
                title="No electronics found matching criteria"
                description="Try clearing your brand, budget or subcategory filters to see all available devices."
                action={
                  <button
                    type="button"
                    onClick={handleClearFilters}
        ...
        {mobileFiltersOpen && (
          <div className="fixed inset-0 z-50 lg:hidden flex justify-end" role="dialog" aria-modal="true">
            ...
            <div className="relative w-full max-w-sm bg-[var(--bg-surface)] h-full overflow-y-auto z-10 p-6 shadow-2xl flex flex-col justify-between animate-slide-left">
              <div>
                <div className="flex items-center justify-between pb-4 mb-4 border-b border-[var(--border-subtle)]">
                  <span className="text-base font-black text-[var(--text-primary)]">Filter Electronics</span>
                  <button
                    type="button"
                    onClick={() => setMobileFiltersOpen(false)}
