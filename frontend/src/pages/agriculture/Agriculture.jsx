...
export const Agriculture = () => {
  ...
  const powerHp = searchParams.get('hp') || '';
  ...
  useEffect(() => {
    const fetchAgri = async () => {
      ...
      try {
        ...
        console.error('Failed to load agriculture listings', err);
        setError('Unable to load agricultural listings at this time.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const farmSubcategories = [
    ...
    'Dairy Cattle, Goats & Livestock',
    'Harvesters, Tillers & Threshers',
    'Organic Seeds, Feed & Crops',
    'Solar Pumps & Micro Irrigation',
    'Farm Storage, Silos & Polyhouses'
  ];

  const hpOptions = [
    { label: '20 - 35 HP (Compact / Mini)', value: '20-35' },
    { label: '35 - 50 HP (Utility)', value: '35-50' },
    { label: '50 - 75 HP (Heavy Duty)', value: '50-75' },
    { label: '75+ HP (Commercial)', value: '75+' }
  ];

  const budgetPresets = [
    { label: 'Under ₹1 Lakh', min: '', max: '100000' },
    { label: '₹1L - ₹5 Lakhs', min: '100000', max: '500000' },
    { label: '₹5L - ₹20 Lakhs', min: '500000', max: '2000000' },
    { label: '₹20L - ₹50 Lakhs', min: '2000000', max: '5000000' },
    { label: '₹50 Lakhs+', min: '5000000', max: '' }
  ];

  const popularRegions = ['All Regions', 'Punjab', 'Haryana', 'Karnataka', 'Maharashtra', 'Telangana', 'Tamil Nadu', 'Gujarat'];
        ...
        <div className="space-y-1.5">
          {hpOptions.map((hp) => (
            <button
              key={hp.value}
              type="button"
              onClick={() => setFilter('hp', powerHp === hp.value ? '' : hp.value)}
              className={cn(
      ...
      <Container size="7xl">
        ...
        <div className="relative mb-8 p-6 sm:p-10 rounded-3xl bg-gradient-to-br from-[var(--bg-surface)] via-[var(--bg-surface)] to-[var(--bg-secondary)] border border-[var(--border-primary)] shadow-sm overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-2">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-lime-500/10 border border-lime-500/20 text-lime-600 dark:text-lime-400 text-xs font-black uppercase tracking-wider">
                <span>🚜</span>
                <span>Verified Agriculture & Farming Marketplace</span>
              </div>
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-[var(--text-primary)] tracking-tight">
                Tractors, Farming Equipment & Acreage
              </h1>
              <p className="text-sm sm:text-base text-[var(--text-secondary)] max-w-2xl leading-relaxed">
                Connect with verified farmers & machinery dealers for Mahindra, John Deere, Swaraj tractors, farmlands, organic feeds, cattle, and solar irrigation systems.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              >
                <span>➕</span>
                <span>Post Farm Listing Free</span>
              </Link>
            </div>
          </div>
          ...
          <div className="relative z-10 mt-8 pt-6 border-t border-[var(--border-subtle)] flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap gap-2">
              {[
                { label: '🌾 All Agriculture', value: '' },
                { label: '🚜 Tractors & Implements', value: 'Tractors & Farm Implements' },
                { label: '📐 Farmland & Plots', value: 'Agricultural Land & Farmland' },
                { label: '🐄 Dairy & Livestock', value: 'Dairy Cattle, Goats & Livestock' },
                { label: '💧 Solar Irrigation', value: 'Solar Pumps & Micro Irrigation' }
              ].map((tab) => (
                <button
                  key={tab.label}
                  type="button"
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1 max-w-full">
              <span className="text-xs font-bold text-[var(--text-muted)] shrink-0 mr-1">Regions:</span>
              {popularRegions.map((r) => {
                const isSelected = (!city && r === 'All Regions') || city === r;
                return (
                  <button
                    key={r}
                    type="button"
                    onClick={() => setFilter('city', r === 'All Regions' ? '' : r)}
                    className={cn(
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 bg-[var(--bg-surface)] p-4 rounded-2xl border border-[var(--border-primary)] shadow-2xs">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="text-xs sm:text-sm font-bold text-[var(--text-secondary)]">
              Showing <span className="text-[var(--text-primary)] font-black">{totalCount}</span> verified agri listings
            </span>
            ...
            {activeFiltersCount > 0 && (
              <div className="hidden md:flex items-center gap-1.5 flex-wrap">
                {subcategory && (
                  <span className="px-2.5 py-0.5 rounded-md bg-lime-500/10 text-lime-600 dark:text-lime-400 border border-lime-500/20 text-[11px] font-bold">
                    {subcategory}
                  </span>
                )}
                {powerHp && (
        ...
        {viewMode === 'map' && (
          <div className="mb-8 rounded-3xl overflow-hidden border border-[var(--border-primary)] shadow-md bg-[var(--bg-surface)] p-2">
            <RealMapLocation
              city={city || 'Punjab'}
              address={`${city || 'Agricultural Belt'}, Farm Equipment & Land Hub`}
              title="Explore Farmlands & Agricultural Equipment on Map"
              interactive={true}
              height="380px"
            />
          </div>
          ...
          <main className="flex-1 w-full min-w-0">
            {loading ? (
              <div className="py-28 flex flex-col items-center justify-center gap-3">
                <Loader size="lg" />
                <span className="text-xs font-bold text-[var(--text-muted)] animate-pulse">Loading verified agricultural listings...</span>
              </div>
            ) : error ? (
              ...
              <EmptyState
                title="No agricultural listings found matching criteria"
                description="Try clearing your budget or machinery filters to see all available agricultural items."
                action={
                  <button
                    type="button"
                    onClick={handleClearFilters}
        ...
        {mobileFiltersOpen && (
          <div className="fixed inset-0 z-50 lg:hidden flex justify-end" role="dialog" aria-modal="true">
            ...
            <div className="relative w-full max-w-sm bg-[var(--bg-surface)] h-full overflow-y-auto z-10 p-6 shadow-2xl flex flex-col justify-between animate-slide-left">
              <div>
                <div className="flex items-center justify-between pb-4 mb-4 border-b border-[var(--border-subtle)]">
                  <span className="text-base font-black text-[var(--text-primary)]">Filter Agriculture</span>
                  <button
                    type="button"
                    onClick={() => setMobileFiltersOpen(false)}
